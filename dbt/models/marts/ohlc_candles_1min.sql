{{
  config(
    ...
    properties={
      "format": "'PARQUET'",
      "partitioned_by": "ARRAY['hour(candle_timestamp)']",
      "sorted_by": "ARRAY['currency_pair', 'candle_timestamp']"
    },
    tags=['marts', 'ohlc', '1min']
  )
}}

/*
  1-minute OHLC candles from FX tick data
  - Aggregates tick data into 1-minute intervals
  - Calculates open, high, low, close prices
  - Tracks volume and trade count
  - Incremental: Only processes new data based on transaction_timestamp
*/
...
with source_ticks as (
    ...
    {% if is_incremental() %}
        ...
        -- Process from 5 minutes before the latest candle to avoid gaps
...
ticks_with_order as (
    select
        date_trunc('minute', transaction_timestamp) as candle_timestamp,
        currency_pair,
        base_currency,
        quote_currency,
        ...
        exchange_location,
        row_number() over (
            partition by currency_pair, date_trunc('minute', transaction_timestamp)
