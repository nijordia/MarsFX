...
/*
  1-hour OHLC candles from FX tick data
  - Aggregates tick data into 1-hour intervals
  - Calculates open, high, low, close prices
  - Tracks volume and trade count
  - Incremental: Only processes new data with 2-hour lookback window
*/
...
with source_ticks as (
    ...
    {% if is_incremental() %}
        ...
        -- Process from 2 hours before the latest candle to avoid gaps
        where date_trunc('hour', transaction_timestamp) >= (
            select max(candle_timestamp) - interval '{{ var("lookback_minutes")["1h"] }}' minute from {{ this }}
        )
    {% endif %}
),
...
ticks_with_order as (
    select
        date_trunc('hour', transaction_timestamp) as candle_timestamp,
        currency_pair,
        base_currency,
        quote_currency,
        ...
        trader_type,
        row_number() over (
            partition by currency_pair, date_trunc('hour', transaction_timestamp)
